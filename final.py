...
- Role-based pages (Dashboard, Control Panel, Admin, Info)
"""
...
# -------------------------
SIM_BATCH_SIZE = 20
MODEL_FILE = "food_quality_model (2).pkl" # 🔹 renamed for safety (remove spaces/brackets)
...
# -------------------------
def get_recommendation(latest_row):
    """Generate corrective action messages based on prediction and sensor values."""
    if latest_row is None:
        return "No data available yet."

    actions = []
    if latest_row["pred_label"] == 1:
        actions.append("⚠️ Product predicted as **DEFECT**. Immediate action required.")
    else:
        actions.append("✅ Product predicted as **GOOD**. Keep monitoring.")

    if latest_row["temp"] > st.session_state.setpoint_temp + 5:
        actions.append("🌡️ Temperature is too high. Reduce heating/cooling load.")
    elif latest_row["temp"] < st.session_state.setpoint_temp - 5:
        actions.append("🌡️ Temperature is too low. Increase heating.")

    if latest_row["humidity"] > st.session_state.setpoint_humidity + 10:
        actions.append("💧 Humidity above safe range. Turn on dehumidifier.")
    elif latest_row["humidity"] < st.session_state.setpoint_humidity - 10:
        actions.append("💧 Humidity too low. Consider humidification.")

    if latest_row["airflow"] < st.session_state.setpoint_airflow - 0.5:
        actions.append("🌬️ Airflow below required level. Check fans/vents.")
    elif latest_row["airflow"] > st.session_state.setpoint_airflow + 0.5:
        actions.append("🌬️ Airflow too strong. Reduce fan speed.")

    if latest_row["vibration"] > st.session_state.setpoint_vibration + 0.2:
        actions.append("⚙️ High vibration detected. Inspect machinery alignment.")
    ...
    if st.session_state.data_source == "Simulated Data":
        ...
        with left:
            ...
            if not df.empty:
                ...
                st.subheader("📢 Predictive Action Recommendations")
                st.info(get_recommendation(latest_row))
            else:
